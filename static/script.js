...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function initialize() {
        ...
        // Carrega projetos do usuário de forma segura
        try {
            await fetchAndDisplayProjects();
        } catch(error) {
    ...
    async function fetchAndDisplayProjects() {
        try {
            const response = await fetch('/get_projects');
            if (!response.ok) throw new Error('Não foi possível obter a lista de projetos.');
            const projects = await response.json();
            projectList.innerHTML = '';
            if (projects.length === 0) {
                projectList.innerHTML = '<li>Nenhum projeto salvo.</li>';
    ...
    function renderWorkbench() {
        workbench.innerHTML = ''; // Limpa completamente para evitar duplicatas
        systemState.components.forEach(comp => {
            const el = document.createElement('div');
            el.className = `placed-component ${comp.type}`;
            el.id = comp.id;
