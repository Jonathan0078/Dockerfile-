document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        if (!response.ok) throw new Error('Falha na resposta do servidor ao buscar banco de dados.');
    ...
    function openModalForComponent(id) {
        ...
        if (modeloSelect) {
            modeloSelect.addEventListener('change', (e) => {
                ...
                const tipoInput = document.getElementById('rolamento-tipo-input');
                const cargaCInput = document.getElementById('rolamento-carga-c-input');
                if (rolamentoData) {
                    tipoInput.value = rolamentoData.tipo;
                    cargaCInput.value = rolamentoData.carga_c;
                } else {
                    tipoInput.value = '';
                    cargaCInput.value = '';
                }
    ...
    // --- 8. FUNÇÕES DE ANÁLISE E OTIMIZAÇÃO ---
    ...
    async function handleOptimizeSubmit(e) {
        ...
        resultsContent.innerHTML = '<h3>Otimizando...</h3><p>Isso pode levar alguns segundos. Estamos testando combinações para você.</p>';
    ...
    function displayOptimizationResults(solutions, goal) {
        ...
        html += '<table style="width:100%; text-align:left; border-collapse: collapse; font-size: 0.8rem;"><thead><tr><th style="border: 1px solid #ddd; padding: 6px;">Configuração</th><th style="border: 1px solid #ddd; padding: 6px;">Custo/Ano</th><th style="border: 1px solid #ddd; padding: 6px;">Vida Útil (h)</th><th style="border: 1px solid #ddd; padding: 6px;">Efic. (%)</th></tr></thead><tbody>';
        solutions.forEach(sol => {
            html += `<tr><td style="border: 1px solid #ddd; padding: 6px;">${sol.config}</td><td style="border: 1px solid #ddd; padding: 6px;">${sol.cost.toLocaleString('pt-BR')}</td><td style="border: 1px solid #ddd; padding: 6px;">${sol.min_life.toLocaleString('pt-BR')}</td><td style="border: 1px solid #ddd; padding: 6px;">${sol.efficiency.toFixed(1)}</td></tr>`;
        });
        html += '</tbody></table>';
