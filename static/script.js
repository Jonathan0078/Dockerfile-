...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Seleciona os botões de ambos os locais possíveis (header e barra móvel)
    ...
    // Adiciona o listener a todos os botões encontrados
    ...
    function handleClearWorkbench() {
        const confirmation = confirm("Tem certeza que deseja limpar toda a bancada? Todos os componentes serão removidos.");
        if (confirmation) {
    ...
    function renderWorkbench() {
        ...
        systemState.components.forEach(comp => {
            ...
            if (comp.type.includes('polia') && comp.data.diameter) {
                ...
                el.style.width = `${Math.max(40, parseFloat(comp.data.diameter) * scaleFactor)}px`;
                el.style.height = `${Math.max(40, parseFloat(comp.data.diameter) * scaleFactor)}px`;
            }
            ...
            el.addEventListener('click', () => { if (!el.classList.contains('dragging')) openModalForComponent(comp.id); });
