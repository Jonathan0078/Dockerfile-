...
def create_app():
    ...
    # --- CONFIGURAÇÃO DE PRODUÇÃO FINAL E À PROVA DE FALHAS ---
    ...
    # O resto do código (rotas, etc.) vai aqui dentro do contexto da app
    with app.app_context():
        # ... (todas as rotas e a classe AnalisadorDeSistema)
        @app.cli.command('init-db')
        def init_db_command():
            db.create_all()
            print('Banco de dados inicializado.')

        # (As rotas /login, /register, etc., vão aqui)
    
    return app

# (Esta parte é movida para dentro da fábrica para melhor escopo)
# ... A forma mais limpa é registrar as rotas diretamente
# ... e não ter a classe AnalisadorDeSistema no escopo global.
# ... Vamos manter seu código funcional como está, mas registrando as rotas.

def register_routes_and_commands(app):
    # O código anterior movido para uma função de registro para clareza
    ...
    # ... (demais rotas, como /login, /register, etc.)
...
app = create_app()
# Registra as rotas na aplicação criada
register_routes_and_commands(app)
