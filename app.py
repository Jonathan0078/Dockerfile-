import math
import json
import itertools # Biblioteca para criar combinações
from flask import Flask, jsonify, render_template, request

app = Flask(__name__)

class AnalisadorDeSistema:
    # --- A CLASSE ANALISADOR DE SISTEMA COMPLETA ---
...
def optimize_system_route():
    try:
        ...
        # Limita o número de combinações para não sobrecarregar o servidor gratuito do Render
        # Cria combinações de polias (motora, movida)
        pulley_combinations = list(itertools.product(possible_pulleys, repeat=2))
        # Cria combinações de rolamentos (assumindo 2 rolamentos)
        bearing_combinations = list(itertools.product(possible_bearings, repeat=2))
        
        # Itera sobre um número limitado de cenários
        for p_combo in pulley_combinations[:50]: # Limita as combinações de polias
            for b_combo in bearing_combinations[:50]: # Limita as combinações de rolamentos
                
                current_system = json.loads(json.dumps(base_system))
                # Substitui os dados dos componentes com a combinação atual
                # (Assume uma ordem fixa: Motor, Polia Motora, Polia Movida, Rolamento, Rolamento)
                if len(current_system['components']) >= 5:
                    current_system['components'][1]['data']['diameter'] = p_combo[0]
                    current_system['components'][2]['data']['diameter'] = p_combo[1]
                ...
                solutions.append({
                    "config": f"Polias {p_combo[0]}/{p_combo[1]} | Rolamentos {b_combo[0]['modelo']}/{b_combo[1]['modelo']}",
                    "cost": cost, "efficiency": efficiency, "min_life": min_life
                })
