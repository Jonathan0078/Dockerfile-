...
class AnalisadorDeSistema:
    ...
    def analisar(self):
        ...
        rpm_motor = float(motor['data']['rpm']); power_kw = float(motor['data']['power_kw']); motor_efficiency = float(motor['data'].get('efficiency', 95)) / 100; cost_per_kwh = float(motor['data'].get('cost_per_kwh', 0.75)); operating_hours_per_day = float(motor['data'].get('operating_hours', 8)); belt_type = polia_motora['data'].get('belt_type', 'V'); d_motora = float(polia_motora['data']['diameter']); d_movida = float(polia_movida['data']['diameter'])
